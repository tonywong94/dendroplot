...
prename = 'GMC1_12CO_12m7m'
casalog = 'casa-20170713-014825.log'
image   = prename + '.image.fits'    # image cube
resid   = prename + '.residual.fits' # residual cube
model   = prename + '.model.fits'    # model cube
mask    = prename + '.mask.fits'     # mask cube
sig_v0  = 235   # min velocity of signal range
sig_v1  = 248   # max velocity of signal range
...
if data_mom is not None:
    ...
    ax1.contour(mask_max, colors='w', levels=[0.5])
...
if resid_mom is not None:
    ...
    cbar.ax.tick_params(labelsize=8) 
    ax2.contour(mask_max, colors='w', levels=[0.5])
...
if model_mom is not None:
    ...
    cbar.ax.tick_params(labelsize=8) 
    ax3.contour(mask_max, colors='w', levels=[0.5])
    ax3.set_title('Model')
...
if os.path.isfile(casalog):
    ...
    # Output parameters
    ax6.text(xleft,ytop-4*ystep,'weighting = {0}'.format(
        tclnpar['weighting']), transform=ax6.transAxes)
    ax6.text(xleft,ytop-5*ystep,'niter = {0}'.format(
        tclnpar['niter']), transform=ax6.transAxes)
    ax6.text(xleft,ytop-6*ystep,'threshold = {0}'.format(
        tclnpar['threshold']), transform=ax6.transAxes)
    ax6.text(xleft,ytop-7*ystep,'deconvolver = {0}'.format(
